...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			// The previous method wasn't adding new accounts to the list, instead it was calling .get() method, which
			// also resulted in NullPointerException being thrown
			// Spotted and fixed during testing of openAccount method
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		// During testing it was noticed that previous if statement triggered only if accountid parameter wasn't in the
		// key list, which resulted in the method throwing NullPointerException in an otherwise scenario.
		// Spotted and fixed during testing of deposit method
		if (!accountlist.containsKey(accountid)) {
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		// During testing it was noticed that withdraw method does not function properly and instead it
		// deposits the money given in parameter. fixed by changing the name of account method reference
		// from account.deposit() to account.withdraw()
		if (!accountlist.containsKey(accountid)) {
			throw new AccountDoesNotExistException();
		}
		else {
	...
	public void addTimedPayment(String accountid, String payid, Integer interval, Integer next, Money amount, Bank tobank, String toaccount) throws AccountDoesNotExistException {
		// During testing an unexpected NullPointerException was noticed, This is due to not checking if account exists
		// in the first place. I changed it so that the method first checks if such account is in the bank, and if so
		// then it schedules the timed payment accordingly. If such account is not present in the bank the method will
